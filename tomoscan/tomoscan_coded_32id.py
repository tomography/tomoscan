...
from nv200.shared_types import PidLoopMode, TriggerInFunction
...
# --- CODED aperture piezo constants ---
# Two Piezosystem Jena NV200/D controllers, one per aperture axis.
# Reference: ~/conda/32id-procedures-decarlof/procedures/nv200_trigger_step.py
IP_X = '10.54.102.8'   # piexo3.xray.aps.anl.gov
IP_Y = '10.54.102.46'  # piexo4.xray.aps.anl.gov
N_POSITIONS = 1024
POS_MIN = 0.0
POS_MAX = 100.0
...
class TomoScanCODED32ID(TomoScanPSO):
    ...
    def __init__(self, pv_files, macros):
        ...
        # CODED aperture piezo state. A persistent asyncio loop is required so
        # device handles created in begin_scan() remain valid in end_scan().
        self._async_loop = asyncio.new_event_loop()
        self._dev_x = None
        self._dev_y = None
        self._wg_x = None
        self._wg_y = None
        self.coded_positions_x = None
        self.coded_positions_y = None
    ...
    def set_trigger_mode(self, trigger_mode, num_images):
        ...
        if(camera_model=='Oryx ORX-10G-51S5M' or camera_model=='Oryx ORX-10G-310S9M'):            
            ...
            self.set_trigger_mode_grasshopper(trigger_mode, num_images)
        elif(camera_model=='C16240-20UP' or camera_model=='ORCA Fire'):
            self.set_trigger_mode_orca_fire(trigger_mode, num_images)    
        else:
    ...
    def set_trigger_mode_orca_fire(self, trigger_mode, num_images):
        self.epics_pvs['CamAcquire'].put('Done') ###
        self.wait_pv(self.epics_pvs['CamAcquire'], 0) ###
        ...
        if trigger_mode == 'FreeRun':
            ...
            self.epics_pvs['CamTriggerMode'].put('Off', wait=True)
            if 'CamTriggerSource' in self.epics_pvs:
                self.epics_pvs['CamTriggerSource'].put('External', wait=True)
            self.epics_pvs['CamImageMode'].put('Multiple')            
    ...
    def set_trigger_mode_grasshopper(self, trigger_mode, num_images):
        ...
        if trigger_mode == 'FreeRun':
            ...
            if 'CamTriggerSource' in self.epics_pvs:
                self.epics_pvs['CamTriggerSource'].put('Line2', wait=True)
            if 'CamTriggerOverlap' in self.epics_pvs:
                self.epics_pvs['CamTriggerOverlap'].put('ReadOut', wait=True)
            if 'CamExposureMode' in self.epics_pvs:
                self.epics_pvs['CamExposureMode'].put('Timed', wait=True)
            self.epics_pvs['CamImageMode'].put('Multiple')            
            if 'CamArrayCallbacks' in self.epics_pvs:
                self.epics_pvs['CamArrayCallbacks'].put('Enable')
            if 'CamFrameRateEnable' in self.epics_pvs:
                self.epics_pvs['CamFrameRateEnable'].put(0)
    ...
    def begin_scan(self):
        ...
        # Set FPGA gate-delay lines to the exposure time. The delayed copy of
        # the PSO pulse arrives at the D200 during detector readout so the
        # piezo steps to the next position between exposures.
        exposure_us = int(self.exposure_time * 1_000_000)
        fpga_prefix = self.epics_pvs['FPGAPVPrefix'].get(as_string=True)
        PV(fpga_prefix + 'GateDly-3_DLY').put(exposure_us, wait=True)
        PV(fpga_prefix + 'GateDly-4_DLY').put(exposure_us, wait=True)
        log.info('FPGA gate delays set to %d us (exposure %.6f s)', exposure_us, self.exposure_time)

        # Fresh random positions each scan, then load + arm both piezos.
        self.coded_positions_x = np.random.uniform(POS_MIN, POS_MAX, N_POSITIONS)
        self.coded_positions_y = np.random.uniform(POS_MIN, POS_MAX, N_POSITIONS)
        self._piezo_arm()
    ...
    def end_scan(self):
        ...
        self.add_theta()
        # Add coded aperture positions to the same HDF file (before scp transfer)
        self.add_coded_aperture_positions()
        ...
        # Stop the piezo waveform generators and release the telnet connections.
        self._piezo_disarm()
    ...
    def add_coded_aperture_positions(self):
        """Write coded aperture x/y positions used per projection into the HDF file.

        Mirrors add_theta(): filters to actually-captured projections via
        NDArrayUniqueId / HDF5FrameLocation, and indexes the 1024-position
        buffer with (proj_ids - proj_ids[0]) % N_POSITIONS so dropped frames
        and >1024-projection scans (which wrap the NV200 waveform loop) are
        both handled the same way.
        """
        if self.coded_positions_x is None or self.coded_positions_y is None:
            log.warning('Coded aperture positions unavailable (piezo arm may have failed); skipping HDF write')
            return

        log.info('add coded aperture positions')
        full_file_name = self.epics_pvs['FPFullFileName'].get(as_string=True)
        if not os.path.exists(full_file_name):
            log.error('Failed adding coded aperture positions. %s file does not exist', full_file_name)
            return
        try:
            with h5py.File(full_file_name, 'a') as f:
                unique_ids = f['/defaults/NDArrayUniqueId']
                ...
                proj_ids = unique_ids[hdf_location[:] == b'/exchange/data']
                if len(proj_ids) > 0:
                    idx = (proj_ids - proj_ids[0]) % N_POSITIONS
                    x_ds = f.create_dataset('/instrument/coded_aperture/x', (len(proj_ids),))
                    x_ds[:] = self.coded_positions_x[idx]
                    y_ds = f.create_dataset('/instrument/coded_aperture/y', (len(proj_ids),))
                    y_ds[:] = self.coded_positions_y[idx]
        except Exception:
            log.error('Add coded aperture positions: Failed accessing: %s', full_file_name)
            traceback.print_exc(file=sys.stdout)

    def _piezo_arm(self):
        """Sync entry point: program random positions into both piezos and arm them."""
        try:
            self._async_loop.run_until_complete(self._async_piezo_arm())
        except Exception:
            log.error('Failed to arm coded-aperture piezos')
            traceback.print_exc(file=sys.stdout)

    async def _async_piezo_arm(self):
        for ip, positions, dev_attr, wg_attr, label in [
            (IP_X, self.coded_positions_x, '_dev_x', '_wg_x', 'X'),
            (IP_Y, self.coded_positions_y, '_dev_y', '_wg_y', 'Y'),
        ]:
            log.info('CODED aperture piezo %s: connecting to %s', label, ip)
            dev = NV200Device(transport=TelnetProtocol(host=ip))
            await dev.connect(auto_adjust_comm_params=True)
            setattr(self, dev_attr, dev)

            await dev.pid.set_mode(PidLoopMode.CLOSED_LOOP)
            wg = WaveformGenerator(dev)
            await wg.set_waveform_buffer(
                buffer=list(positions),
                unit=WaveformUnit.POSITION,
            )
            await wg.configure_waveform_loop(
                start_index=0,
                loop_start_index=0,
                loop_end_index=N_POSITIONS - 1,
            )
            await dev.set_trigger_function(TriggerInFunction.WAVEFORM_STEP)
            await wg.start(cycles=0, start_index=0)
            setattr(self, wg_attr, wg)
            log.info('CODED aperture piezo %s: %d positions armed', label, N_POSITIONS)

    def _piezo_disarm(self):
        """Sync entry point: stop, disable trigger, and disconnect both piezos."""
        try:
            self._async_loop.run_until_complete(self._async_piezo_disarm())
        except Exception:
            log.error('Failed to disarm coded-aperture piezos')
            traceback.print_exc(file=sys.stdout)

    async def _async_piezo_disarm(self):
        for dev_attr, wg_attr, label in [
            ('_dev_x', '_wg_x', 'X'),
            ('_dev_y', '_wg_y', 'Y'),
        ]:
            dev = getattr(self, dev_attr)
            wg = getattr(self, wg_attr)
            if wg is not None:
                try:
                    await wg.stop()
                except Exception as e:
                    log.warning('CODED piezo %s: wg.stop() failed: %s', label, e)
            if dev is not None:
                try:
                    await dev.set_trigger_function(TriggerInFunction.DISABLED)
                except Exception as e:
                    log.warning('CODED piezo %s: set_trigger DISABLED failed: %s', label, e)
                try:
                    await dev.close()
                except Exception as e:
                    log.warning('CODED piezo %s: close failed: %s', label, e)
            setattr(self, dev_attr, None)
            setattr(self, wg_attr, None)
